...
export const GeneralPerformance = () => {
  const { data, isLoading } = useQuery({
    queryKey: ["ga4_traffic_campaign_monthly_yoy"],
    queryFn: async () => {
  ...
  const adaptedData = data?.rows?.map((item: any, index: number) => ({
    id: index,
    total_users: item.total_usuarios,
    diff_total_users: item.diff_pct_total_users,
    new_users: item.new_users,
    diff_new_users: item.diff_pct_new_users,
    sessions: item.sessions,
    diff_sessions: item.diff_pct_sessions,
    engaged_sessions: item.engaged_sessions,
    diff_engaged_sessions: item.diff_pct_engaged_sessions,
    average_session_duration: item.duracion_media_sesion,
    diff_average_session_duration: item.diff_duracion_media_sesion,
    bounce_rate: item.tasa_rebote,
    diff_bounce_rate: item.diff_abs_tasa_rebote,
    key_events: item.evento_clave,
    diff_key_events: item.diff_pct_evento_clave,
    key_event_rate: item.tasa_evento_clave,
    diff_key_event_rate: item.diff_abs_tasa_evento_clave,
  }));
